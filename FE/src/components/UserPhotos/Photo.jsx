...
function Photo({ photos, users, userIdMe, handleUpdatePhotos, code, token }) {
  ...
  const handleDeletePost = async (e) => {
    ...
    // Hiện ra hộp thoại xác nhận khi người dùng nhấn nút xóa
    ...
    try {
      await axios.delete(
        `http://localhost:8080/api/user/deleteimage/${photos._id}`,
        config,
      );
      handleUpdatePhotos();
    } catch (err) {
  ...
  const handleCommentPost = async (e) => {
    ...
    try {
      setError("");
      await axios.post(
        `http://localhost:8080/api/user/commentsOfPhoto/${photos._id}`,
        {
          comment: comment,
          user_id: userIdMe,
        },
  ...
  return (
    ...
    >
      ...
      <div>
        <ListItemText
          primary={`Upload time: ${photos.date_time.slice(0, 10)} ${photos.date_time.slice(11, 19)}`}
          secondary={
            <React.Fragment>
              <Typography
                component="span"
